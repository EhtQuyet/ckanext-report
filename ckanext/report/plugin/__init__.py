...
from ckanext.report import actions, db
from ckanext.report import auth
...
def build_pages_nav_main(*args):
    ...
    about_menu = tk.asbool(tk.config.get('ckanext.report.about_menu', True))
    group_menu = tk.asbool(tk.config.get('ckanext.report.group_menu', True))
    org_menu = tk.asbool(tk.config.get('ckanext.report.organization_menu', True))
    ...
    if ckan_29_or_higher:
        ...
        is_current_page = (
            ...
            and tk.c.controller == 'ckanext.report.controller:ReportController')
    if is_current_page:
...
def render_content(content):
    allow_html = tk.asbool(tk.config.get('ckanext.report.allow_html', False))
    return tk.h.render_markdown(content, allow_html=allow_html)


def get_wysiwyg_editor():
    return tk.config.get('ckanext.report.editor', '')
...
class ReportPluginBase(p.SingletonPlugin, DefaultTranslation):
    p.implements(p.ITranslation, inherit=True)


class ReportPlugin(ReportPluginBase, MixinPlugin):
    ...
    def update_config(self, config):
        self.organization_pages = tk.asbool(config.get('ckanext.report.organization', False))
        self.group_pages = tk.asbool(config.get('ckanext.report.group', False))
    ...
    def get_actions(self):
        actions_dict = {
            ...
            'ckanext_report_list': actions.pages_list,
        }
        if self.organization_pages:
            org_actions = {
    ...
    def get_auth_functions(self):
        return {
            ...
            'ckanext_report_list': auth.report_list,
