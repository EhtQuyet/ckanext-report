...
INIT_PY = os.path.join(HERE, 'ckanext', 'report', '__init__.py')
version = None
...
setup(
    ...
    entry_points="""
        ...
        textboxview=ckanext.report.plugin:TextBoxView
        [babel.extractors]
        ckan = ckan.lib.extract:extract_ckan
        [paste.paster_command]
        report = ckanext.report.commands:ReportCommand
    """,
    message_extractors={
        'ckanext': [
            ('**.py', 'python', None),
            ('**.js', 'javascript', None),
            ('**/report/theme/**.html', 'ckan', None),
        ],
    },
)
