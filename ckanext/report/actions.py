...
from ckanext.report.logic.schema import update_pages_schema

import ckan.authz as authz

from ckanext.report import db
...
def pages_show(context, data_dict):
    print('context, data_dict', context, data_dict)
    try:
...
@tk.side_effect_free
def report_list(context, data_dict):
    try:
        p.toolkit.check_access('ckanext_report_list', context, data_dict)
    except p.toolkit.NotAuthorized:
        p.toolkit.abort(401, p.toolkit._('Not authorized to see this page'))
    return _pages_list(context, data_dict)
