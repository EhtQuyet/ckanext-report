...
from ckanext.report.download import download
...
def report_pages(page_type, data=None):
    data_dict = {'org_id': None, 'page_type': page_type}
    tk.c.pages_dict = tk.get_action('ckanext_report_list')(
    ...
    vars = {'data': data, 'page_type': page_type, }
    if tk.request.method == 'POST':
        download.test_download()
        return tk.render('ckanext_report/report_table.html', extra_vars=vars)
    return tk.render('ckanext_report/report_table.html', extra_vars=vars)
...
def pages_list_pages(page_type):
    ...
    if page_type == 'blog':
        return tk.render('ckanext_report/blog_list.html')
    return tk.render('ckanext_report/pages_list.html')
...
def pages_edit(page=None, data=None, errors=None, error_summary=None, page_type='pages'):
    ...
    if tk.request.method == 'POST' and not data:
        ...
        if ckan_29_or_higher:
            ...
            return tk.redirect_to('report.%s' % endpoint, page=page_dict['name'])
        else:
    ...
    form_snippet = config.get('ckanext.report.form', 'ckanext_report/base_form.html')
    ...
    return tk.render(
        'ckanext_report/%s_edit.html' % page_type, extra_vars=vars)
...
def pages_show(page=None, page_type='page'):
    ...
    return tk.render('ckanext_report/%s.html' % page_type)
...
def pages_delete(page, page_type='pages'):
    ...
    if 'cancel' in tk.request.params:
        if ckan_29_or_higher:
            return tk.redirect_to('report.%s_edit' % page_type, page=page)
        else:
    ...
    try:
        if tk.request.method == 'POST':
            ...
            if ckan_29_or_higher:
                ...
                return tk.redirect_to('report.%s' % endpoint)
            else:
    ...
    return tk.render('ckanext_report/confirm_delete.html', {'page': page})
...
def group_list_pages(id, group_type, group_dict=None):
    ...
    return tk.render(
        'ckanext_report/{}_page_list.html'.format(group_type),
        extra_vars={
...
def group_show(id, group_type, page=None):
    ...
    return tk.render(
        'ckanext_report/{}_page.html'.format(group_type),
        {
            'group_type': group_type,
            'group_dict': group_dict
        }
...
def group_edit(id, group_type, page=None, data=None, errors=None, error_summary=None):
    ...
    if tk.request.method == 'POST' and not data:
        ...
        if ckan_29_or_higher:
            endpoint = 'report.{}_pages_show'.format(group_type)
    ...
    return tk.render(
        'ckanext_report/{}_page_edit.html'.format(group_type), extra_vars=vars)
...
def group_delete(id, group_type, page):
    ...
    if 'cancel' in tk.request.params:
        if ckan_29_or_higher:
            return tk.redirect_to('report.%s_edit' % group_type, id=tk.c.group_dict['name'], page=page)
        else:
    ...
    try:
        if tk.request.method == 'POST':
            ...
            if ckan_29_or_higher:
                endpoint = 'report.{}_pages_index'.format(group_type)
    ...
    return tk.render(
        'ckanext_report/confirm_delete.html',
        {'page': page, 'group_type': group_type, 'group_dict': group_dict}
    )


def initdb():
    import ckanext.report.db as db
    db.init_db()
